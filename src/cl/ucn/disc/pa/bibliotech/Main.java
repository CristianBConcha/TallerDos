...
public final class Main {
    ...
    private static void calificarLibro(Sistema sistema) {
        ...
        System.out.println("Ingrese isbn del libro a reseñar");
        String libroAReseñar = StdIn.readLine();

        System.out.println("Ingrese una calificacion para el libro de 0 a 5");
        float calificacion = Float.parseFloat(StdIn.readLine());
        if (calificacion <= 5 || calificacion <= 0) {
            if (sistema.calificacionLibro(calificacion, libroAReseñar)) {
    ...
    private static void cambiarContrasenia(Sistema sistema) {
        ...
        //Se envia un mensaje dependiendo de si se logro o no el cambio de contraseña
        if (sistema.contraseniaCambiada(contraseniaNueva)) {
            System.out.println("contraseña cambiada");
        }
        if (!sistema.contraseniaCambiada(contraseniaNueva)) {
            System.out.println("contraseña no ha podido ser cambiada");
        }
    }
    ...
    private static void editarCorreo(Sistema sistema) {
        ...
        //Se envia un mensaje dependiendo de si se logro o no el cambio de contraseña
        if (sistema.correoCambiado(correoNuevo)) {
            System.out.println("correo cambiado");
        }
        if (!sistema.correoCambiado(correoNuevo)) {
            System.out.println("correo no ha podido ser cambiada");
        }
    }
